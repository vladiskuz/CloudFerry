...
class TransportInstance(action.Action):
    ...
    def deploy_instance(self, one_instance):
        ...
        new_ids = {}
        try:
            new_id = dst_compute.deploy(instance)
    ...
    def _deploy_instance_on_random_host(self, one_instance):
        one_instance = copy.deepcopy(one_instance)
        dst_compute = self.dst_cloud.resources[utils.COMPUTE_RESOURCE]

        _instance = one_instance[utils.INSTANCES_TYPE].values()[0]
        instance_body = _instance[utils.INSTANCE_BODY]
        az = instance_body['availability_zone']
        hosts = dst_compute.get_compute_hosts(availability_zone=az)
        if not hosts:
            message = ("No hosts in availability zone '{az}' "
                       "found on destination.").format(az=az)
        ...
        while hosts:
            ...
            # Reassociate floating ips with ports
            ...
            next_host = hosts.pop()
            availability_zone = ':'.join([az, next_host])
            ...
            try:
                new_id = dst_compute.deploy(updated_instance_body, **kwargs)
